...
with h5py.File(args.vecs_fname, 'r') as f:
    vecs = f['vecs'][:,:args.num_vecs]
    weights = np.diagonal(f['cov_weights'][:])[:args.num_vecs]
print('Processing', vecs.shape[1], 'modes')
args.num_vecs = vecs.shape[1]
try:
    assert vecs.shape[0] == atoms.n_atoms * 3
except AssertionError as e:
    raise AssertionError('Number of atoms of PDB and vecs file do not match. Confirm selection string.') from e
...
with open(args.output_fname, 'w') as f:
    ...
    w.writerow(['title', args.vecs_fname])
    w.writerow(['names'] + list(atoms.names))
    w.writerow(['resnames'] + list(atoms.resnames))
    w.writerow(['resnums'] + list(atoms.resids - atoms.resids.min()))
    w.writerow(['coordinates'] + list(atoms.positions.ravel()))
    for i in range(args.num_vecs):
        w.writerow(['mode', str(i+1), str(weights[i])] + list(vecs[:,i]))
